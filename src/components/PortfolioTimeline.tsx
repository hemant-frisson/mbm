...
export default function PortfolioTimeline({
	...
	const handleBack = useCallback(() => {
		if (viewState.view === "images") {
			setViewState({
				view: "bookings",
				monthData: (viewState as any).monthData,
			});
		} else {
	...
	return (
		...
		>
			...
			<div
				...
				style={{
					background:
						"radial-gradient(circle, rgba(201,168,124,0.15) 0%, transparent 70%)",
					top: "10%",
					left: "20%",
			...
			<div
				...
				style={{
					background:
						"radial-gradient(circle, rgba(212,160,160,0.12) 0%, transparent 70%)",
					bottom: "15%",
					right: "10%",
					...
					<div className="flex items-center gap-4">
						<Link href="/" className="flex items-center gap-3 group">
							<div className="w-9 h-9 rounded-full border-2 border-[#c9a87c]/60 flex items-center justify-center group-hover:border-[#e8d5b7] transition-colors duration-300">
								<span className="text-xs font-bold tracking-wider text-[#c9a87c] group-hover:text-[#e8d5b7] transition-colors">
									M
								</span>
							</div>
							<span className="text-sm font-semibold tracking-[0.15em] text-[#f5f0eb] hidden md:block">
								MBM
							</span>
						</Link>

						<svg
							className="w-4 h-4 text-[#3a3530]"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
						...
						<div className="flex items-center gap-2">
							...
							{viewState.view !== "months" && "monthData" in viewState && (
								<>
									...
									>
										{(viewState as any).monthData.monthName}{" "}
										{(viewState as any).monthData.year}
									</button>
								</>
							)}
										...
										<div className="aspect-[4/3] relative overflow-hidden">
											...
											<div className="absolute top-3 right-3 px-2.5 py-1 bg-[#0a0a0a]/50 backdrop-blur-sm rounded-full border border-white/10">
												<span className="text-[11px] text-[#c9a87c] font-semibold">
													{monthData.count}{" "}
													{monthData.count === 1 ? "booking" : "bookings"}
												</span>
											</div>
											...
											<div
												...
												style={{
													background:
														"radial-gradient(circle at 50% 80%, rgba(201,168,124,0.15) 0%, transparent 60%)",
												}}
											/>
										</div>
										...
										<div className="p-4 bg-[#111111]">
											<div className="flex items-end justify-between">
												...
												<div className="w-8 h-8 rounded-full border border-[#c9a87c]/20 flex items-center justify-center group-hover:border-[#c9a87c]/60 group-hover:bg-[#c9a87c]/10 transition-all duration-300">
													<svg
														className="w-4 h-4 text-[#c9a87c] transition-transform duration-300 group-hover:translate-x-0.5"
														fill="none"
														stroke="currentColor"
														viewBox="0 0 24 24"
													>
											...
											<div className="flex flex-wrap gap-1.5 mt-3">
												{Array.from(
													new Set(monthData.bookings.map((b) => b.category)),
												).map((cat) => (
													<span
														key={cat}
														className="px-2 py-0.5 text-[9px] uppercase tracking-wider text-[#6b6560] border border-[rgba(255,255,255,0.06)] rounded-full"
													>
														{cat}
													</span>
												))}
						...
						>
							...
							<div className="aspect-[4/3] relative overflow-hidden">
								...
								<div className="absolute bottom-3 right-3 flex items-center gap-1.5 px-2.5 py-1 bg-[#0a0a0a]/60 backdrop-blur-sm rounded-full border border-white/10">
									<svg
										className="w-3.5 h-3.5 text-[#c9a87c]"
										fill="none"
										stroke="currentColor"
										viewBox="0 0 24 24"
									>
									...
									<span className="text-xs text-[#f5f0eb] font-medium">
...
function ImagesView({
	...
	useEffect(() => {
		const handler = (e: KeyboardEvent) => {
			if (e.key === "ArrowLeft") {
				setSelectedIndex(
					(prev) => (prev - 1 + booking.images.length) % booking.images.length,
				);
	...
	return (
		<div className="max-w-6xl mx-auto px-6 pt-8 pb-24">
			...
			<div className="mb-8 animate-fade-up">
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
					...
					<div className="text-right">
						<div className="text-[10px] text-[#6b6560] uppercase tracking-wider mb-1">
							Makeup by
						</div>
						<div className="text-sm font-semibold text-[#c9a87c]">
							Meena Bisht
						</div>
					</div>
				</div>
			...
			{booking.images.length > 1 && (
				<div
					className="flex gap-3 overflow-x-auto pb-4 no-scrollbar animate-fade-up"
					...
					{booking.images.map((img, i) => (
						<button
							...
							className={`relative flex-shrink-0 w-24 aspect-[4/3] rounded-lg overflow-hidden border-2 transition-all duration-300 ${
								selectedIndex === i
									? "border-[#c9a87c] scale-105"
									: "border-[rgba(255,255,255,0.06)] opacity-50 grayscale hover:opacity-100 hover:grayscale-0"
							}`}
						>
							<Image
								src={img.imageUrl}
